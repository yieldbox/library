...
contract NODERewardManagement {
    ...
    uint256 public claimDuration = 30;
    uint256 public minClaimTax = 10;
    uint256 public maxClaimTax = 20;
    ...
    function createNode(
        ...
        for (uint256 i = 0; i < total; i++) {
            if (i == 0) {
                ...
                string memory nodeName_ = string(
                    abi.encodePacked(nodeName, "_", Strings.toString(i + 1))
                );
                _nodesOfUser[account].push(
                    NodeEntity({
                        name: nodeName_,
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 fee = 0;
        if (node.lastClaimTime + claimDuration <= block.timestamp) {
            fee += rewardNode.mul(minClaimTax).div(100);
        } else {
            fee += rewardNode.mul(maxClaimTax).div(100);
        }
        rewardNode -= fee;
        nodes[index].lastClaimTime = block.timestamp;
        nodes[index].rewardAvailable = 0;

        return (rewardNode, fee);
    }

    function updateClaimDurationTime(uint256 newClaimTime) external onlySentry {
        claimDuration = newClaimTime;
    }

    function updateMaxClaimTax(uint256 value) external onlySentry {
        maxClaimTax = value;
    }

    function updateMinClaimTax(uint256 value) external onlySentry {
        minClaimTax = value;
    }
        ...
        returns (uint256, uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            ...
            if (nodes[i].lastClaimTime + claimDuration <= block.timestamp) {
                fee += rewardsTotal.mul(minClaimTax).div(100);
            } else {
                fee += rewardsTotal.mul(maxClaimTax).div(100);
            }
            rewardsTotal -= fee;
    ...
    function _getNodeAvailableRewardAmountOf(
        address account,
        uint256 creationTime
        ...
        returns (string memory)
    {
        ...
        string memory _rewardsAvailable = uint2str(
            _getNodeAvailableRewardAmountOf(account, nodes[0].creationTime)
        );
        string memory separator = "#";
        ...
        for (uint256 i = 1; i < nodesCount; i++) {
            ...
            _rewardsAvailable = string(
                abi.encodePacked(
                    ...
                    separator,
                    uint2str(
                        _getNodeAvailableRewardAmountOf(
                            account,
                            _node.creationTime
                        )
                    )
    ...
    function creditLastNode(address account, uint256 amount)
        external
        onlySentry
    {
        NodeEntity[] storage nodes = _nodesOfUser[account];
        nodes[0].rewardAvailable += amount;
    }
...
contract Simian is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    uint256 public numberOfLoops = 20;
    ...
    constructor(
        ...
        address uniV2Router,
        uint256 swapAmount
    ) ERC20("Simian Nodes", "SIMIAN") PaymentSplitter(payees, shares) {
    ...
    function createNodeWithTokens(string memory name, uint256 total) public {
        require(
        ...
        require(
            balanceOf(sender) >= nodePrice.mul(total),
            "NODE CREATION: Balance too low for creation."
        );
        uint256 totalUserNodes = nodeRewardManager.getUserNodes(sender);
        require(
            totalUserNodes.add(total) <= 100,
        ...
        super._transfer(sender, address(this), total.mul(nodePrice));
        nodeRewardManager.createNode(sender, name, total);
    }

    function cashoutMultipleReward(uint256[] memory blocktimes) public {
        address sender = _msgSender();
        ...
        require(blocktimes.length <= 10, "Maximum number of 10");
        uint256 totalReward = 0;
        for (uint256 i = 0; i < blocktimes.length; i++) {
            totalReward += nodeRewardManager._getRewardAmountOf(
                sender,
                blocktimes[i]
            );
        }
        ...
        if (swapLiquify) {
            uint256 feeAmount;
            if (cashoutFee > 0) {
                feeAmount = totalReward.mul(cashoutFee).div(100);
                swapAndSendToFee(futurUsePool, feeAmount);
            }
            totalReward -= feeAmount;
        }
        super._transfer(distributionPool, sender, totalReward);
    }
    ...
    function cashoutAll() public {
        ...
        (uint256 rewardAmount, uint256 taxFee) = nodeRewardManager
        ...
        if (swapLiquify) {
            ...
            totalFee += feeAmount;
            swapAndSendToFee(futurUsePool, totalFee);
            rewardAmount -= totalFee;
        }
    ...
    function updateNumberOfLoops(uint256 amount) external onlyOwner {
        numberOfLoops = amount;
    }

    function autoCompound(uint256[] memory blocktimes, string memory name)
        external
    {
        address sender = _msgSender();
        require(blocktimes.length <= 40, "Maximum selection exceeded");
        uint256 totalUserNodes = nodeRewardManager.getUserNodes(sender);
        require(totalUserNodes < 100, "Maximum nodes reached");
        uint256 total = 0;
        for (uint256 i = 0; i < 10; i++) {
            (uint256 rewardAmount, uint256 fee) = nodeRewardManager
                ._cashoutNodeReward(sender, blocktimes[i]);
            total += rewardAmount.add(fee);
        }
        uint256 nodePrice = nodeRewardManager.nodePrice();
        uint256 real_total = total.div(nodePrice);
        uint256 remainder = total.sub(real_total);
        require(total >= nodePrice, "Balance not enough to compound");
        nodeRewardManager.createNode(sender, name, real_total);
        if (remainder > 0) {
            nodeRewardManager.creditLastNode(sender, remainder);
        }
    }
}
