...
contract Playmates is IERC20 {
    ...
    string private constant _name = "Playmates";
    string private constant _symbol = "PLAYMATES";
    ...
    Fees public _taxRates = Fees({buyFee: 0, sellFee: 1000, transferFee: 4000});
    ...
    address payable public marketingWallet =
        payable(0x8C400D07C258e07681587D3DBDc3dF1CE3306DD7);
    address payable private rewardsPool =
        payable(0xA3b4C11E2625b3A39c838DA649A28B00F3c49cCE);
    address payable private treasuryWallet =
        payable(0xE63fb42e89cD67d93bcb6DC5E9388B347E3174be);
    ...
    function getWhaleFee(address from) public view returns (uint256) {
        if (mansion.balanceOf(from) >= 1 && mansion.balanceOf(from) < 20) {
            return vBuy1;
        }
        if (mansion.balanceOf(from) >= 20 && mansion.balanceOf(from) < 50) {
            return vBuy2;
        }
        if (mansion.balanceOf(from) >= 50 && mansion.balanceOf(from) < 100) {
            return vBuy3;
        }
        if (mansion.balanceOf(from) >= 100) {
            return vBuy4;
        } else {
            return 0;
        }
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        if (address(this).balance > 0) {
            ...
            treasuryWallet.transfer(
                (amountAVAX * _ratios.treasury) /
                    (_ratios.treasury + _ratios.marketing)
            );
