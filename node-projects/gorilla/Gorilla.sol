...
GorillaNodes Nodes Manager.
Websitet: https://gorillanodes.finance
Discord: https://discord.io/GorillaNodes
Twitter: https://twitter.com/gorillanodes
...
contract GorillaNodes is ERC20, Ownable, PaymentSplitter {
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "GorillaNodes::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
                if (sender == joePair) {
                    require(
                        balanceOf(recipient).add(amount) <= maxBalanceAmount(),
                        "GorillaNodes::antiWhale: Transfer would exceed the maxBalanceAmount of the recipient"
                    );
                }
            }
        }
    ...
    constructor(
    ...
    ) ERC20("GorillaNodes", "BANANA") PaymentSplitter(payees, shares) {
        ...
        _mint(_msgSender(), 20010000 * 10**18);
    }
        ...
        onlyOwner
    {
        require(
            ...
            "GorillaNodes::updatemaxBalanceAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
        require(
            _maxBalanceAmountRate >= 200,
            "GorillaNodes::updatemaxBalanceAmountRate: Max transfer amount rate must  exceed the minimum rate."
        );
        ...
        onlyOwner
    {
        require(
            ...
            "GorillaNodes::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
        require(
            _maxTransferAmountRate >= 100,
            "GorillaNodes::updateMaxTransferAmountRate: Max transfer amount rate must exceed the minimum rate."
        );
    ...
    function createNodeWithAirdropAdmin(string memory name, address destination)
        external
        onlyOwner
    {
        require(bytes(name).length > 3 && bytes(name).length < 32, "NC:1");
        require(destination != address(0), "NC:2");
        require(!isBlacklisted[destination], "BLACKLISTED");
        require(destination != teamPool && destination != rewardsPool, "NC:4");

        nodeManager.createNode(destination, name, 10 * 10**18);
    }

    function multipleNodeAirdrop(address[] calldata addresses)
        public
        onlyOwner
    {
        for (uint256 i; i < addresses.length; ++i) {
            ...
            require(
                addresses[i] != teamPool && addresses[i] != rewardsPool,
                "NC:4"
            );
        ...
        if (swapLiquifyEnabled) {
            ...
            if (NFTboosterActivated) {
                ...
                if (((block.timestamp - last_booster_claim) >= 24 hours)) {
                    uint256 numberboosternft = boosterNFTaddress.balanceOf(
                        sender
                    );
                    if (numberboosternft > 0) {
                        ...
                        if (numberboosternft > usernodes) {
                            add_reward = usernodes.mul(25e16);
                            valid_NFT = usernodes;
                        } else if (numberboosternft == usernodes) {
                            add_reward = numberboosternft.mul(25e16);
                            valid_NFT = usernodes;
                        } else {
                            add_reward = numberboosternft.mul(25e16);
                        ...
                        booster_economy = full_tax.mul(valid_NFT).div(
                            usernodes
                        );
