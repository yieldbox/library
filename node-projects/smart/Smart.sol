...
contract SmartNodes is ERC20, Ownable, PaymentSplitter {
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "SmartNodes:: AntiWhale: Transfer amount exceeds the maxTransferAmount"
                );
                if (sender == joePair) {
                    require(
                        balanceOf(recipient).add(amount) <= maxBalanceAmount(),
                        "SmartNodes: AntiWhale: Transfer would exceed the maxBalanceAmount of the recipient"
                    );
                }
            }
        }
    ...
    modifier antiSniper(
        ...
        if (
            !_excludedFromAntiSniper[from] &&
            !_excludedFromAntiSniper[to] &&
            !_excludedFromAntiSniper[callee]
        )
            require(
                !(size1 > 0 || size2 > 0 || size3 > 0),
                "SmartNodes: Sniper Detected"
            );
        _;
    }
    ...
    constructor(
        ...
        uint256[] memory fees,
        uint256 swapAmount
    ) ERC20("SmartNodes", "SMRT") PaymentSplitter(payees, shares) {
        ...
        require(
            joeRouterAddress != address(0),
            "SmartNodes: Invalid JoeRouter Address"
        );
    ...
    function setExcludedFromTaxes(address _account, bool value)
        external
        onlyOwner
    {
    ...
    function setExcludedFromAntiSniper(address _account, bool _excluded)
        public
        onlyOwner
    {
        ...
        onlyOwner
    {
        require(
            ...
            "SmartNodes: updatemaxBalanceAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
        require(
            _maxBalanceAmountRate >= 200,
            "SmartNodes: updatemaxBalanceAmountRate: Max transfer amount rate must  exceed the minimum rate."
        );
        ...
        onlyOwner
    {
        require(
            ...
            "SmartNodes: updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
        require(
            _maxTransferAmountRate >= 100,
            "SmartNodes: updateMaxTransferAmountRate: Max transfer amount rate must exceed the minimum rate."
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SmartNodes: Error");
    ...
    function _transfer(
        ...
        bool isTransfer = from != joePair &&
            from != joeRouterAddress &&
            to != joePair &&
            to != joeRouterAddress;
    ...
    function _distributeTax(uint256 taxAmount, address from) internal {
        ...
        if (
            ...
            uint256 refReceived = taxAmount
                .sub(rewardsPoolTokens)
                .sub(treasuryRewardstoSwap)
                .sub(teamTokens);
    ...
    function getSalesTax(uint256 _timestamp) public view returns (uint256) {
        if (_timestamp <= sTaxLim) return salesTax1;
        else return salesTax2;
    }

    function getTransferTax(uint256 _timestamp) public view returns (uint256) {
        if (_timestamp <= tTaxLim) return transferTax1;
        else return transferTax2;
    }

    function updateTaxes(uint256[] memory taxes, uint256[] memory taxPercents)
        external
        onlyOwner
    {
        salesTax1 = taxes[0];
    ...
    function updateTaxesPeriod(uint256 taxLim1, uint256 taxLim2)
        external
        onlyOwner
    {
        sTaxLim = block.timestamp + taxLim1;
    ...
    function addLiquidityJR(uint256 tokenAmount, uint256 ethAmount)
        public
        payable
        onlyOwner
    {
        ...
        notPaused
    {
        ...
        require(amount_ >= nodeValue, "SmartNodes: Not enough SMN tokens");
        require(
            bytes(name).length > 3 && bytes(name).length < 32,
            "SmartNodes: Invalid Name"
        );
        require(sender != address(0), "SmartNodes: Wrong address");
    ...
    function cashoutReward(uint256 blocktime) external {
        ...
        require(rewardAmount > 0, "SmartNodes: No reward to cash out");
    ...
    function cashoutAll() external {
        ...
        if (swapLiquifyEnabled) {
            ...
            if (cashoutFee > 0) {
                ...
                uint256 rewardsPoolCut = feeAmount.mul(rewardsPoolPercent).div(
                    100
                );
    ...
    function updateCashoutDistribution(
        uint256 _rewardsPoolCut,
        uint256 _teamPoolCut
    ) public onlyOwner {
    ...
    function compoundNodeRewards(uint256 blocktime) external {
        ...
        uint256 rewardAmount = nodeManager.getNodeReward(sender, blocktime);
        require(rewardAmount > 0, "SmartNodes: No rewards to compound");
        uint256 initBal1 = balanceOf(address(this));
    ...
    function airdropTokens(address _account, uint256 _amount)
        external
        onlyOwner
    {
