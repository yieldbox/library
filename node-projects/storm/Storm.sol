...
interface INodeManager {
    ...
    function createNodeAllInOne(
        address account,
        string memory nodeName,
        uint256 amount_
    ) external;

    function multipleCreateNode(
        address account,
        string memory nodeName,
        uint256 amount,
        uint256 timeStamp
...
contract StormZv2 is ERC20, Ownable {
    ...
    uint256[] public cashoutFeeTiers = [
        1000,
        900,
        800,
        700,
    ...
    IERC20 public OLD_CONTRACT =
        IERC20(0xb952Df7188AC2F81eee0C4C9aAAde6Ec4a5eeA06);
    INodeManagerOLD private OLD_MANAGER =
        INodeManagerOLD(0x7C2A55031318F636593B7824A8D95443Ead23210);

    IERC20 public boosterNFTaddress =
        IERC20(0x35F1DeD4b275Bf3Ac06804B9AE413f40C2a8B121);
    ...
    mapping(address => uint256) public userLastBlockBuyTransactions;
    ...
    constructor(address[] memory addresses) ERC20("STORMZv2", "STORMZv2") {
    ...
    function updateCashoutFeeTiers(uint256[] calldata newVal)
        external
        onlyOwner
    {
    ...
    function updateTiersRequiredDays(uint256[] calldata newVal)
        external
        onlyOwner
    {
        require(newVal.length == 11, "Wrong length");
        ...
        if (to == joePair) {
            ...
            if (from == joePair && sender != owner()) {
                if (maxWalletActivated) {
                    require(
                        amount + balanceOf(sender) <= maxWalletToken,
                        "Total holding is limited"
                    );
                }
            }
            super._transfer(from, to, amount);
    ...
    function createMultipleNodeWithTokens(uint256 amount_, uint256 nodeamount_)
        external
    {
        address sender = _msgSender();
        require(sender != address(0), "NC:2");
        ...
        for (uint256 i = 1; i <= nodeamount_; i++) {
            nodeManager.multipleCreateNode(
                sender,
                uint2str(nodeNumber),
                amount_,
                loopTimeStamp + i
    ...
    function migrateNodesAllInOne() external {
        ...
        INodeManagerOLD.NodeEntity[] memory _nodes = OLD_MANAGER.getAllNodes(
            sender
        );
        totalNodeValue = 0;
        ...
        if (_nodes.length > 0) {
            ...
            if (_nodes.length > 100) {
                for (uint256 i = 1; i <= 100; i++) {
                    nodeManager.multipleCreateNode(
                        sender,
                        concatenate("STORMZ_V1-", uint2str(i)),
                        _nodes[i - 1].amount,
                        migrateTime + i
                    );
                    nodeNumber++;
    ...
    function cashoutAll() external {
        ...
        if (_lastClaim[sender] > 0) {
            elapsedTimeInMinutes =
                (block.timestamp - _lastClaim[sender]) /
                1 minutes;
        } else {
            elapsedTimeInMinutes =
                (block.timestamp - launchedAtTime) /
                1 minutes;
        }
        ...
        if (NFTboosterActivated) {
            ...
            if (numberboosternft > 0) {
                ...
                if (numberboosternft > usernodes) {
                    add_reward = usernodes
                        .mul(boostPerNFT)
                        .mul(elapsedTimeInMinutes)
                        .div(NFTboosterReset);
                    valid_NFT = usernodes;
                } else if (numberboosternft == usernodes) {
                    add_reward = numberboosternft
                        .mul(boostPerNFT)
                        .mul(elapsedTimeInMinutes)
                        .div(NFTboosterReset);
                    valid_NFT = usernodes;
    ...
    function calculateCashoutFee(uint256 _elapsedTime)
        internal
        view
        returns (uint256)
    {
    ...
    function getUserBoost(address sender) public view returns (uint256) {
        ...
        if (getNumberboosternft > 0) {
            ...
            if (getNumberboosternft > getUsernodes) {
                getBoost = getUsernodes
                    .mul(boostPerNFT)
                    .mul(getElapsedTimeInMinutes)
                    .div(NFTboosterReset);
            } else if (getNumberboosternft == getUsernodes) {
                getBoost = getNumberboosternft
                    .mul(boostPerNFT)
                    .mul(getElapsedTimeInMinutes)
                    .div(NFTboosterReset);
            } else {
